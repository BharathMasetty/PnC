...
NaoMainController::NaoMainController(
    NaoTaskAndForceContainer* _taf_container, RobotSystem* _robot) {
  myUtils::pretty_constructor(2, "Nao Main Controller");
  // Initialize Flag
  b_first_visit_ = true;
  ...
  act_list.resize(Nao::n_dof, true);
  for (int i(0); i < Nao::n_vdof; ++i) act_list[i] = false;
  wbc_ = new WBC(act_list);

  tau_cmd_ = Eigen::VectorXd::Zero(Nao::n_adof);
  qddot_cmd_ = Eigen::VectorXd::Zero(Nao::n_adof);

  // Initialize Joint Integrator
  wbc_dt_ = NaoAux::servo_rate;
  joint_integrator_ = new JointIntegrator(Nao::n_adof, wbc_dt_);

  // Initialize desired pos, vel, acc containers
  des_jpos_ = Eigen::VectorXd::Zero(Nao::n_adof);
  des_jvel_ = Eigen::VectorXd::Zero(Nao::n_adof);
  des_jacc_ = Eigen::VectorXd::Zero(Nao::n_adof);
}
...
void NaoMainController::_PreProcessing_Command() {
...
void NaoMainController::getCommand(void* _cmd) {
  ...
  joint_integrator_->integrate(
      des_jacc_, sp_->qdot.segment(Nao::n_vdof, Nao::n_adof),
      sp_->q.segment(Nao::n_vdof, Nao::n_adof), des_jvel_, des_jpos_);
  ...
  for (int i(0); i < Nao::n_adof; ++i) {
    ((NaoCommand*)_cmd)->jtrq[i] = tau_cmd_[i];
    ((NaoCommand*)_cmd)->q[i] = des_jpos_[i];
    ((NaoCommand*)_cmd)->qdot[i] = des_jvel_[i];
  }
  // std::cout << "In Main controller: " << des_jpos_[27] << std::endl;
}

void NaoMainController::firstVisit() {
  // Initialize joint integrator
  Eigen::VectorXd jpos_ini = sp_->q.segment(Nao::n_vdof, Nao::n_adof);
  joint_integrator_->initializeStates(Eigen::VectorXd::Zero(Nao::n_adof),
                                      jpos_ini);
}

void NaoMainController::ctrlInitialization(const YAML::Node& node) {
  // WBC Defaults
  wbc_dt_ = NaoAux::servo_rate;
