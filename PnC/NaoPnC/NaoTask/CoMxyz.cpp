...
bool CoMxyz::_UpdateTaskJacobian() {
  ...
  // FLAG: No pelvis for Nao --> Use Torso and clarify later

  // Eigen::MatrixXd Jtmp =
  // robot_->getBodyNodeCoMJacobian(NaoBodyNode::torso);
  Eigen::MatrixXd Jtmp = robot_->getCoMJacobian();
  // (X, Y, Z)
  Jt_.block(0, 0, 3, robot_->getNumDofs()) =
      Jtmp.block(3, 0, 3, robot_->getNumDofs());

  // Replace Z Jacobian with torso
  Jtmp = robot_->getBodyNodeCoMJacobian(NaoBodyNode::torso);
